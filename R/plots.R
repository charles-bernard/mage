  ...
  for(i in 1:n_scores) {
    for(j in 1:n_scores) {
      if(j <= i) {
        ...
        par(mfg = c(i, j));
        plot(scores_mat[, j], scores_mat[, i], col = "blue", pch = 16, cex = .5, ann = F);
        if(j == 1)
          mtext(colnames(scores_mat)[i], side = 2, cex = 0.7, padj = -4);
        if(i == j)
          mtext(colnames(scores_mat)[j], side = 3, cex = 0.7, padj = -1);
      }
    }
  }
